#!/usr/bin/env python3
"""
Phase 6A: Download FWS and APHIS comment headers.

Reuses the Phase 3 infrastructure but only targets FWS and APHIS.
These agencies were skipped in the original Phase 3 run.

FWS: ~2.5M comments
APHIS: ~561K comments
Estimated time: ~12-15 hours at 1000 req/hr
"""
...
# === Logging ===
log = logging.getLogger("fws_aphis_headers")
log.setLevel(logging.INFO)
_fmt = logging.Formatter("%(asctime)s [%(levelname)s] %(message)s")
_fh = logging.FileHandler(LOG_DIR / "fws_aphis_headers.log")
...
def download_comments_day(session, agency, year, month, day):
    """Download comments for a single day."""
    params = {
...
def download_comments_month(session, agency, year, month):
    """Download comments for a single month. Subdivides to days if truncated."""
    ...
    if saved < 0:
        # Month exceeds pagination limit — subdivide by day
        total_avail = -saved
        log.warning(f"    [{agency}] {year}-{month:02d}: {total_avail} comments exceed {MAX_PAGE * PAGE_SIZE} limit, subdividing by day")
        saved = 0
        for day in range(1, last_day + 1):
            ...
            if cnt < 0:
                log.warning(f"    [{agency}] {year}-{month:02d}-{day:02d}: {-cnt} even in single day, got max {MAX_PAGE * PAGE_SIZE}")
...
def download_agency_comments(session, agency, state):
    ...
    for year in range(START_YEAR, current_year + 1):
        ...
        progress(f"FWS/APHIS headers: {agency} {year} done — {agency_total} comments so far, {state['total_comments']} total")
...
def main():
    log.info("=" * 60)
    log.info("FWS/APHIS COMMENT HEADERS — Starting")
    log.info(f"Agencies: {', '.join(AGENCIES)}")
    log.info(f"Rate limit: {MIN_INTERVAL}s between requests ({int(3600/MIN_INTERVAL)}/hr)")
    log.info("Estimated: ~12-15 hours for ~3M comments")
    log.info("=" * 60)
    progress("FWS/APHIS headers: STARTING")
    ...
    for agency in AGENCIES:
        ...
        log.info(f"{'='*40}")
        progress(f"FWS/APHIS headers: starting {agency}")
        ...
        progress(f"FWS/APHIS headers: {agency} done — {state['total_comments']} total comments so far")

    # Summary
    status = "Interrupted" if _shutdown else "Complete"
    log.info("=" * 60)
    log.info(f"FWS/APHIS COMMENT HEADERS — {status}")
    log.info(f"Total comment headers (all agencies): {state['total_comments']}")
    log.info(f"API calls this session: {rate.count}")
    log.info("=" * 60)
    progress(f"FWS/APHIS headers: {status.upper()} — {state['total_comments']} total, {rate.count} API calls this session")
